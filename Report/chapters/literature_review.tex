...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.21\textwidth}p{0.32\textwidth}p{0.34\textwidth}}
        ...
        Wire-flame physics, heterogeneous experimental predictors, categorisation, augmentation, and ML FSR benchmarks &
...
Their baseline using numerical predictors alone produced weaker performance than versions that included one-hot encoded categorical variables. Under categorised random ten-fold cross-validation, several nonlinear models attained mean coefficients of determination around or above \(0.90\); the exact values vary by model and augmentation strategy \cite{riveraWireML}. These are literature results. They cannot be transferred directly to this project's database, targets, partitions, or code, and they do not establish held-paper generalisation.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{pinn_example.png}
  \caption{Example physics-informed neural-network solution from Raissi et al.:
  sparse initial and boundary observations and interior collocation points
  constrain the nonlinear Schrödinger solution; predicted profiles (red dashed)
  are compared with the exact solution (blue). Reproduced from Fig.~1 of
  \cite{raissi2019pinn}.}
  \label{fig:pinn-example}
\end{figure}

Raissi et al.\ demonstrate the formulation on nonlinear Schrödinger, Allen--Cahn, Navier--Stokes, Korteweg--de Vries, and Burgers equations, using continuous- and discrete-time variants. They emphasize that PINNs complement rather than replace mature numerical methods and identify unresolved issues involving architecture, optimisation, convergence, noise, and uncertainty \cite{raissi2019pinn}. Karniadakis et al.\ broaden the discussion to scientific settings where observational data may be scarce, noisy, or incomplete and prior physical knowledge can constrain an otherwise large hypothesis space \cite{karniadakis2021physicsinformed}.
...
\section{Synthesis, research gap, and objectives}
...
\subsection{Research gap and data-driven motivation}

Reduced gravity changes the balance among diffusion, imposed ventilation, radiation, and solid conduction. Experiments are correspondingly expensive, and the published evidence is fragmented across materials, geometries, facilities, atmospheres, and reporting conventions. A finite qualification matrix cannot densely sample this space, while a high-fidelity reacting-flow model requires boundary conditions and material properties that are often absent from historical reports.

The research gap is therefore not simply a lack of another predictive algorithm. Three specific absences remain. The \emph{target gap} is that the closest machine-learning study concentrates on FSR, whereas an engineering screening workflow also needs an explicit ignition model. The \emph{validation gap} is that randomly splitting rows can place nearly repeated observations from one rig on both sides of the evaluation boundary, creating optimistic evidence for a material-selection question that usually concerns a new campaign. Random cross-validation within a compiled literature table does not measure transfer to a held-out paper, apparatus, or condition regime. The \emph{integration gap} is that broad spacecraft studies establish operational relevance while wire and tabular-learning studies establish local predictive feasibility, but the evidence has not yet been organised into one traceable, leakage-resistant evaluation of both ignition and FSR.

A data-driven response is motivated by this fragmentation rather than by a claim that combustion physics is obsolete. What is missing is a unified database and an evaluation that asks both whether models interpolate among conditions from known campaigns and whether they transfer to a completely held-out paper. That distinction is the difference between recovering nearby points in a familiar apparatus and supporting a screening decision for a new material, geometry, or laboratory.
...
\begin{enumerate}
  ...
  \item interpret model behaviour using held-out diagnostics, permutation
  importance, and SHAP without treating predictive associations as causal laws.
\end{enumerate}

The intended role is screening and experiment prioritisation, not replacement of qualification tests or combustion physics. Only ignition and FSR are modelled. Extinction, flame length, HRR, and smoke/aerosol prediction remain future modelling tasks. This boundary preserves a clear distinction between the database's definition of flammability and the narrower scope of the validated machine-learning evidence. The project predicts recorded ignition and FSR outcomes; it does not certify flight materials or solve the full reacting-flow problem. A reliable material-selection tool must make clear both what has been learned from existing tests and where a new experiment remains the only defensible next step.
